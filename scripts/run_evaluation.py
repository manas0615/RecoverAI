...
class HybridLiveGeminiStrategy(EvaluationStrategy):
    ...
    def evaluate(self, evidence: ObservableCaseEvidence) -> StrategyDecision:
        ...
        try:
            _risk, cause, plan = analyzer.analyze(case, events)
            decision = self.policy_engine.evaluate(
                self.policy_context, case, plan, action_history, cause
            )

            if decision.decision == PolicyDecisionValue.APPROVE:
                action = (
                    plan.selected_action_type.value
            ...
            _risk, cause, plan = fallback_analyzer.analyze(case, events)
            decision = self.policy_engine.evaluate(
                self.policy_context, case, plan, action_history, cause
            )
            if decision.decision == PolicyDecisionValue.APPROVE:
            ...
            return StrategyDecision(
                ...
                proposed_action=plan.selected_action_type.value
                if plan.selected_action_type
                else "SUPPRESS",
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hybrid-ai", action="store_true", help="Run hybrid AI evaluation"
    )
    parser.add_argument(
        "--llm-limit", type=int, default=10, help="Number of real Gemini calls to make"
    )
    parser.add_argument(
        "--seed", type=int, default=42, help="Random seed for scenarios"
    )
    ...
    if args.hybrid_ai:
        ...
        print("--- Benchmark Validation Run ---")
        print(
            f"Seed: {results['metadata']['seed']}, Scenarios: {results['metadata']['scenario_count']}"
        )
