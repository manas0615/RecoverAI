...
def test_closed_loop_recovery_trigger():
    ...
    with container.tm.transaction() as conn:
        conn.execute(
            "INSERT OR IGNORE INTO merchants (merchant_id, display_name, default_currency, status, created_at, updated_at) VALUES ('merch_1', 'Test Merchant', 'INR', 'ACTIVE', '2023-01-01', '2023-01-01')"
        )
    ...
    failure_event = RevenueEvent(
        ...
        metadata={
            "payload": {
                "payment": {"entity": {"description": f"Recovery Action {action_id}"}}
            }
        },
        occurred_at=datetime.now(UTC),
        received_at=datetime.now(UTC),
    ...
    with container.tm.transaction() as conn:
        ...
        from recoverai.domain.action import ActionType, RecoveryAction, RecoveryActionId
