...
def setup_dummy_merchant_and_case(tm: TransactionManager) -> RecoveryCaseId:
    with tm.transaction() as conn:
        conn.execute("INSERT INTO merchants (merchant_id, external_reference, display_name, status, default_currency, created_at, updated_at) VALUES ('merch_1', 'ext', 'name', 'ACTIVE', 'INR', '2026-08-26T00:00:00+00:00', '2026-08-26T00:00:00+00:00')")
        conn.execute("INSERT INTO revenue_events (event_id, event_type, source_type, merchant_id, metadata, schema_version, occurred_at, received_at) VALUES ('evt_1', 'PAYMENT_FAILED', 'API', 'merch_1', '{}', '1.0', '2026-08-26T00:00:00+00:00', '2026-08-26T00:00:00+00:00')")
...
def test_terminal_state_protection(tm):
    ...
    cmd = CloseCaseCommand(
        case_id=case_id,
        outcome=RecoveryOutcomeValue.SUPPRESSED,
        timestamp=dt
    )
    ...
    cmd2 = CloseCaseCommand(
        case_id=case_id,
        outcome=RecoveryOutcomeValue.ESCALATED,
        timestamp=dt
    )
    with pytest.raises(TerminalStateError):
