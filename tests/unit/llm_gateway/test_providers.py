...
from recoverai.llm_gateway.providers import HuggingFaceAdapter, ProviderError, ConfigurationError
...
def test_hf_authentication_failure(mock_urlopen):
    ...
    with pytest.raises(ConfigurationError, match="Authentication/Configuration failed: 401"):
        adapter.generate_json("prompt", {})
...
def test_hf_timeout(mock_urlopen):
    ...
    with pytest.raises(ProviderError, match="Hugging Face API failed: TimeoutError - timeout"):
        adapter.generate_json("prompt", {})
...
def test_hf_503_unavailable(mock_urlopen):
    ...
    mock_urlopen.side_effect = urllib.error.HTTPError(
        url="", code=503, msg="Service Unavailable", hdrs={}, fp=MagicMock(read=lambda: b"{}")
    )
...
def test_engine_provider_selection():
    config = GatewayConfig(
        gemini_api_key="gem", groq_api_key="groq", hf_api_key="hf",
