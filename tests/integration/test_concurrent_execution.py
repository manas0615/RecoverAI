...
from recoverai.domain.action import RecoveryAction, ActionStatus, ActionType
...
from recoverai.domain import MerchantId, CustomerId, RecoveryCaseId, RecoveryActionId, RevenueEventId
...
def test_concurrent_execution():
    ...
    with tm.transaction() as conn:
        conn.execute("INSERT OR IGNORE INTO merchants (merchant_id, display_name, default_currency, status, created_at, updated_at) VALUES ('merch_demo', 'Demo Merchant', 'USD', 'ACTIVE', '2023-01-01', '2023-01-01')")
        conn.execute("INSERT OR IGNORE INTO customers (customer_id, merchant_id, display_name, created_at, updated_at) VALUES ('cust_demo', 'merch_demo', 'Demo Customer', '2026-01-01T00:00:00Z', '2026-01-01T00:00:00Z')")
        ...
        event = RevenueEvent(
            ...
            source=EventSource(source_type=EventSourceType.RAZORPAY_WEBHOOK, source_event_id=f"test_{uuid.uuid4().hex}"),
    ...
    def worker():
        with tm.transaction() as conn:
            ...
            action._real_plan = InterventionPlan.from_dict(json.loads(action.plan_snapshot))
    ...
    assert mock_execute.call_count == 1, f"Concurrency race detected! Provider called {mock_execute.call_count} times."
