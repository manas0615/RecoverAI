...
from recoverai.domain.assessment import CauseAssessment, RiskAssessment, AnalysisType
...
def test_analyze_path_systemic_degradation(base_case):
    ...
    with mock.patch.object(container.intelligence, "analyze") as mock_analyze:
        ...
        mock_cause = CauseAssessment(
            ...
            analysis_type=AnalysisType.RULE_BASED, model_version="mock",
            created_at=datetime.now(UTC)
        )
        ...
        response = client.post(f"/recovery-cases/{case_id.value}/analyze", headers={"X-API-Key": "test_frontend_key_default"})
        assert response.status_code == 200
        
        from recoverai.persistence.repositories.audit import AuditRepository
        with container.tm.transaction() as conn:
            audit_events = [ae for ae in AuditRepository(conn).get_by_case(case_id.value) if ae.event_type.value == "POLICY_DECISION_CREATED"]
...
def test_create_payment_link_execution_path(mock_rzp_urlopen, base_case):
    ...
    # Mock intelligence to return SYSTEMIC_DEGRADATION cause
    with mock.patch.object(container.mcp_context.intelligence, "analyze") as mock_analyze:
        mock_risk = RiskAssessment(
            assessment_id="risk_1",
            case_id=case_id,
            recovery_probability=Probability(0.2, "Low"),
        ...
        req = CreatePaymentLinkInput(case_id=case_id.value, action_id=f"act_{datetime.now(UTC).timestamp()}")
