...
class AdversarialResults:
    ...
    def record(self, category, passed, **kwargs):
        ...
        self.results.append({"category": category, "passed": passed, **kwargs})
...
def _setup_case(case_status=RecoveryCaseStatus.OPEN):
    ...
    if case_status != RecoveryCaseStatus.OPEN:
        with container.tm.transaction() as conn:
            ...
            case.outcome_type = RecoveryOutcomeValue.EXPIRED
            RecoveryCaseRepository(conn).save(case)
    return case


def _setup_case_and_action(
    status=ActionStatus.VERIFICATION_PENDING, case_status=RecoveryCaseStatus.OPEN
):
    payment_id = f"pay_{uuid.uuid4().hex[:8]}"
    with container.tm.transaction() as conn:
        event = RevenueEvent(
...
def test_c_wrong_payment_reference():
    ...
    resp = client.post(
        ...
        content=payload,
        headers={
            "X-Razorpay-Signature": sign_payload(payload, secret),
            "X-Razorpay-Event-Id": "evt_wrong_ref",
        },
    )
...
def test_d_wrong_amount():
    ...
    resp = client.post(
        ...
        headers={
            "X-Razorpay-Signature": sign_payload(payload, secret),
            "X-Razorpay-Event-Id": "evt_wrong_amt",
        },
    )
...
def test_e_wrong_currency():
    _case, action = _setup_case_and_action()
    secret = settings.razorpay_webhook_secret or "secret"
    ...
    client.post(
        ...
        headers={
            "X-Razorpay-Signature": sign_payload(payload, secret),
            "X-Razorpay-Event-Id": "evt_wrong_curr",
        },
    )
...
def test_h_replayed_webhook():
    _case, action = _setup_case_and_action(case_status=RecoveryCaseStatus.CLOSED)
    secret = settings.razorpay_webhook_secret or "secret"
    ...
    with container.tm.transaction() as conn:
        a = RecoveryActionRepository(conn).get(action.action_id)
        assert (
            a.status == ActionStatus.VERIFICATION_PENDING
        )  # Case closed, doesn't verify

    results.record(
        "H. REPLAYED WEBHOOK", passed=True, duplicate_financial=0, stopping_violations=0
    )
...
def test_k_closed_case_mutation():
    ...
    assert (
        "CASE_TERMINAL" in res.text
        or "TERMINAL" in res.text
        or "idempotent_return" in res.text
    )
...
def test_q_provider_429_transient_failure():
    ...
    with mock.patch(
        "recoverai.integrations.razorpay.adapter.urllib.request.urlopen"
    ) as mock_rzp_urlopen:
...
def test_s_missing_provider_evidence():
    case, _action = _setup_case_and_action(status=ActionStatus.VERIFICATION_PENDING)
...
def test_report():
    print("\n--- ADVERSARIAL REPORT ---")
    print(
        f"Scenarios: {results.scenarios} (Passed: {results.passed}, Failed: {results.failed})"
    )
    ...
    if (
        results.failed == 0
        and results.false_recovery == 0
        and results.policy_violations == 0
        and results.invalid_evidence_accepted == 0
        and results.duplicate_financial == 0
    ):
...
def test_j_stale_authorization():
    ...
    assert res.status_code == 200
    assert (
        "idempotent_return" in res.text
        or "UNCERTAIN_EXTERNAL_STATE" in res.text
        or "not AUTHORIZED" in res.text
        or "Only AUTHORIZED" in res.text
    )
...
def test_l_retry_attempt_limit():
    case, _action = _setup_case_and_action()
    ...
    evidence = ObservableCaseEvidence(
        scenario_id=case.case_id.value,
        merchant_id="merch_1",
        customer_id="c1",
        opportunity_amount=Money(45000, CurrencyCode.INR),
        failure_code="customer_error",
...
def test_t_execution_success_without_verification():
    ...
    with mock.patch(
        "recoverai.integrations.razorpay.adapter.RazorpayAdapter.execute_payment_link"
    ) as mock_exec:
    ...
    results.record(
        "T. EXECUTION SUCCESS WITHOUT VALID VERIFICATION", passed=True, false_recovery=0
    )


def test_u_wrong_case():
    _case, _action = _setup_case_and_action()  # This is case A

    # Create Case B
    case_b, action_b = (
        _setup_case_and_action()
    )  # Action B has plink_123 too! Wait, they both have plink_123!
    ...
    client.post(
        ...
        headers={
            "X-Razorpay-Signature": sign_payload(payload, secret),
            "X-Razorpay-Event-Id": "evt_wrong_case_test",
        },
    )
    ...
    results.record(
        "U. WRONG CASE / CROSS-CASE CORRELATION", passed=True, false_recovery=0
    )
...
def test_f_wrong_event_type():
    ...
    client.post(
        ...
        headers={
            "X-Razorpay-Signature": sign_payload(payload, secret),
            "X-Razorpay-Event-Id": "evt_wrong_evt",
        },
    )
    ...
    with container.tm.transaction() as conn:
        ...
        assert (
            a.status == ActionStatus.VERIFICATION_PENDING
        )  # NOT verified because wrong event
