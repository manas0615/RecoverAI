...
def test_real_razorpay_e2e_recovery(client):
    ...
    # Find our new case
    case = next((c for c in cases if c["customer_id"] == customer_id and c["amount_minor"] == amount), None)
    assert case is not None, "Case was not created"
    case_id = case["case_id"]
    
    # 3. Analyze Case (simulate Gemini or Deterministic fallback)
    resp = client.post(f"/recovery-cases/{case_id}/analyze", headers={"X-API-Key": settings.frontend_api_key})
