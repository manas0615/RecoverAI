...
def build_case_and_events(evidence: ObservableCaseEvidence):
    case = RecoveryCase(
        ...
        amount_at_risk=RevenueAmount(
            Money(evidence.opportunity_amount.amount_minor, CurrencyCode.INR)
        ),
        ...
        events.append(
            RevenueEvent(
                ...
                metadata={
                    ...
                    "gateway_downtime": evidence.gateway_downtime_active,
                },
...
class L3CurrentRecoverAI(EvaluationStrategy):
    ...
    def evaluate(self, evidence: ObservableCaseEvidence) -> StrategyDecision:
        ...
        _risk, cause, plan = self.analyzer.analyze(case, events)
        ...
        proposed = (
            plan.selected_action_type.value if plan.selected_action_type else "SUPPRESS"
        )
