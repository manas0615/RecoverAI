...
class OutcomeSimulator:
    def simulate(self, scenario: SyntheticScenario, decision: StrategyDecision) -> dict:
        ...
        can_succeed = (
            ...
            and not scenario.truth.provider_error_on_execution
        )
        ...
        if decision.action_taken == "CREATE_PAYMENT_LINK" and (  # noqa: SIM102
            scenario.evidence.gateway_downtime_active
            or scenario.evidence.opportunity_amount.amount_minor > 40000_00
            or scenario.evidence.failure_code == "fraud_suspected"
        ):
...
class Evaluator:
    def __init__(self, simulator: OutcomeSimulator | None = None):
        self.simulator = simulator or OutcomeSimulator()

    def evaluate(
        self, strategy: EvaluationStrategy, scenarios: list[SyntheticScenario]
    ) -> EvaluationMetrics:
        ...
        for scenario in scenarios:
            ...
            if decision.action_taken == "CREATE_PAYMENT_LINK":
                metrics.expected_recovery_value += amount * Decimal(
