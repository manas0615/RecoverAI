...
def run_test():
    ...
    with tm.transaction() as conn:
        ...
        events = [
            e for e in (event_repo.get(eid) for eid in case.source_event_ids) if e
        ]

        print(
            f"Executing Razorpay on {case.case_id.value} for {case.amount_at_risk.amount_minor} {case.amount_at_risk.currency.value}"
        )
