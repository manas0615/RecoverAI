...
# We want to replace the try block inside analyze_case
# that has: with container.tm.transaction() as conn: 
# all the way to aise HTTPException(status_code=500, detail="Analysis unavailable")

target_start = content.find('    try:\n        from datetime import UTC, datetime')
target_end = content.find('        raise HTTPException(status_code=500, detail="Analysis unavailable")')
target_end += len('        raise HTTPException(status_code=500, detail="Analysis unavailable")')
...
# And we also need the closed case guard right before the try block
